...
def createLegends():
    ...
    # Cm (nel paper i valori sono in picoF qui in nanoF !!!!)
...
def computeRates(voi, states, constants):
    ...
    ### Quindi algebraic sono variabili che poi servono ad inizializzare i rates
...
def computeAlgebraic(constants, states, voi):
    ...
    # Perchè devo ricalcolare le algebraic?
...
def maleckar_model( #V_0=-74.031982,
    ...
    def init_p():
        ...
        constants[35] = 10
        constants[36] = 24.7
        constants[37] = conc_Na
        constants[38] = conc_Ca2
        constants[39] = conc_K
        constants[40] = iupMax
        constants[41] = Kcyca
        constants[42] = Ksrca
        constants[43] = Kxcs
        constants[44] = alpha_rel
        states[25] = 0.632613
        states[26] = 0.649195
        constants[45] = 0.0003969
        constants[46] = 0.0000441
    ...
    (init_states, constants) = init_p()

    # Set timespan to solve over
    # voi = linspace(0, 10, 500) Per prima cosa riduco per studiare effettivamente la variazione nel tempo
    voi = linspace(0, 0.5, 100)
...
def plot_model(voi, states, algebraic):
    ...
    pylab.figure(2)
    pylab.plot(voi,states.T)
    pylab.xlabel(legend_voi)
    #pylab.legend(legend_states + legend_algebraic, loc='best')
    pylab.show()

    pylab.figure(3)
    pylab.plot(voi,algebraic.T)
    pylab.xlabel(legend_voi)
    #pylab.legend(legend_states + legend_algebraic, loc='best')
    pylab.show()
...
# Potremmo fare orima uno screening visuale
